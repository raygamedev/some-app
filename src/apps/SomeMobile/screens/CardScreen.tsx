...
export const CardScreen = () => {
  ...
  useEffect(() => {
    const getCardData = async () => {
      ...
      if (guid != null) {
        setUserGuid(guid);
        let card =
          (await apiGetUserCard(guid)) || (await apiGenerateCard(guid));
        if (card === null) {
  ...
  const onBarcodeScanSuccess = async ({
    data,
      ...
      <PopupModal open={openRedeemPopup}>
        <View style={styles.redeemPopup}>
          <Text style={{ fontSize: 17 }}>
            Read the following code to the merchant
          </Text>
